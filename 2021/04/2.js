  ...
  (input) => {
    ...
    for (let i = 5; i < numbers.length; i++) {
      ...
      const remainingBoards = boards.filter(
        (board) => !winners.includes(board),
      );
      const nextWinners = remainingBoards.filter((board) => {
        return getSolutionSets(board).some((set) => isSubset(progress, set));
      });
      if (nextWinners.length) {
        winners.push(...nextWinners);
      }
      if (winners.length === boards.length) {
        const finalBoard = winners[winners.length - 1];
        const cells = getAllCells(finalBoard);
